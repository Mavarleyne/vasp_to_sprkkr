...
def generate_sys(path: Path):
    ...
    with open(output_filename, "w") as f:
        ...
        for i, site in enumerate(structure_prim.sites):
            ...
            filestring += f"{i + 1:3d} {i + 1:3d} {pos[0]:16.12f} {pos[1]:16.12f} {pos[2]:16.12f} {rws:20.12f}   3    1  {ito:2d}\n"
        ...
        if len(labels) == len(set(labels)):
            ...
            for i, (el, sym) in enumerate(zip(dict.fromkeys(elements).keys(), dict.fromkeys(labels).keys())):

                filestring += f" {it:2d} {z:3d}  {label:4s}   {len(iq_sites):2d}  1.000  {' '.join(map(str, iq_sites))}\n"


        # flag = True
        # for i, (el, label) in enumerate(zip(structure_prim.sites, labels)):
        #     z = el.specie.Z
        #     if num_sites[label] == 1 and flag:
        #         flag = False
        #
        #         filestring += f" {it:2d} {z:3d}  {label:4s}   {len(num_sites[label]):2d}  1.000  {' '.join(map(str, iq_sites))}\n"


        for i, (el, it) in enumerate(zip(elements, unique_indexes)):
            z = el.Z
            label = labels[i]
            if len(labels) == len(unique_elements):
