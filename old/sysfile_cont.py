...
from ase2sprkkr.physics.winger_seitz_radii import winger_seitz_radii
...
def sysfile_content(atoms, filename='<unknown>'):
    ...
    class TypeData(object):
        ...
        def __repr__(self):
            s = "<TypeData(\'{}\', atomic_number={:d}, concentration={:.4f}, site={:d},equivalent_sites={})>".format(
                self.symbol, self.atomic_number, self.concentration, self.it, self.iqs)
            return s
    ...
    s = ""
    rws = winger_seitz_radii
    angmom = 3
    for i, sd in enumerate(sites):
        filestring += "%3i%4i%18.12f%18.12f%18.12f  %18.12f%4i%5i " % (
        sd.iq, sd.icl, sd.coors[0], sd.coors[1], sd.coors[2], rws[sd.site.occupation.primary_symbol], angmom, len(sd.occupancy))
...
if __name__ == '__main__':
    old_p = f'for_spr/216/Zr8Nb8/martensite/0/POSCAR'
    pos = Poscar.from_file(Path(f'for_spr/216/Cd4Re8Ru4/austenite/0/POSCAR'))
